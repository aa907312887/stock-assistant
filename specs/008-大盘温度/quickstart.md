...
1. 打开首页，确认“我的投资逻辑”右侧出现大盘温度模块。  
2. 检查温度为5档之一，且有对应文案。  
3. 鼠标悬停档位，出现策略提示。  
4. 点击“?”按钮，弹出计算口径说明。  
5. 切换移动端模式，确认主区有当日单行操作建议，策略说明弹层可读；近 20 日为彩色方块：上行档位名、下行温度分；五档并列「档位：操作」全文仅在「?」中查看。  

### US1 验收补充

- 首页“我的投资逻辑”右侧可见大盘温度卡片
- 卡片展示：分值、档位、更新时间
...
- 后端接口：`latest/trend/explain/rebuild` 已实现（待联调验证）
- 前端模块：首页卡片与口径弹层已接入（待浏览器回归）
