...
- **定时任务**：若功能涉及定时任务（如每日拉数、定时同步），必须明确写清：使用什么组件及在项目中的位置；如何注册（在应用启动的何处、何时注册）；调度策略（cron 表达式或具体时间）；**部署时是否执行一次**及触发方式；**是否提供手动触发方式**（HTTP 接口/管理命令/脚本及鉴权）；失败与重试、日志与告警。
- **接口与数据流**：写清请求路径、主要参数、响应结构、错误约定；数据从哪来到哪去、经哪些层。
