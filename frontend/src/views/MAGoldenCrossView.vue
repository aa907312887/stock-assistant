<template>
  <div class="page">
    <div class="header">
      <div>
        <div class="titleRow">
          <span class="title">均线金叉</span>
          <el-tooltip placement="bottom-start" :show-after="200">
            <template #content>
              <div class="tipBlock">
                <p>本页按日线扫描全 A 股，列出截止日满足「均线金叉」触发条件的股票。</p>
                <p><strong>金叉条件</strong>：当日 MA5 > MA10，且前一日 MA5 ≤ MA10（首次上穿）</p>
                <p><strong>多头排列</strong>：MA5 > MA10 > MA20 且三者都比前一天高</p>
                <p><strong>成交量放大</strong>：当日成交量 ≥ 前一日 × 1.5</p>
                <p><strong>低位约束</strong>：收盘价 ≤ 历史最高价的 2/3</p>
                <p>不构成投资建议。</p>
              </div>
            </template>
        ...
        <div class="subtitle">
          MA5 首次上穿 MA10、均线多头排列、成交量放大、处于相对低位；触发日收盘确认，次日低开由您自行判断是否买入。
        </div>
      </div>
      <div class="actions">
        <el-date-picker
    ...
    <el-card class="card" shadow="never">
      ...
      <div class="note">
        当前仅筛选满足金叉条件和多头排列的股票，不判断次日低开条件，由您自行观察次日开盘情况决定是否买入。可手动选择日期执行选股。
      </div>
    </el-card>
    ...
    <el-card class="card" shadow="never">
      ...
      <el-table :data="filteredItems" v-loading="loading" stripe style="width: 100%; margin-top: 12px">
        ...
        <el-table-column prop="trigger_date" label="金叉日" width="120" />
        <el-table-column label="MA5" width="90" align="right">
          <template #default="{ row }">{{ fmtNum(row.summary?.ma5) }}</template>
        </el-table-column>
        <el-table-column label="MA10" width="90" align="right">
          <template #default="{ row }">{{ fmtNum(row.summary?.ma10) }}</template>
        </el-table-column>
        <el-table-column label="MA20" width="90" align="right">
          <template #default="{ row }">{{ fmtNum(row.summary?.ma20) }}</template>
        </el-table-column>
...
const STRATEGY_ID = 'ma_golden_cross'
