<template>
  <div class="page">
    <div class="header">
      <div>
        <div class="titleRow">
          <span class="title">连续打板</span>
          <el-tooltip placement="bottom-start" :show-after="200">
            <template #content>
              <div class="tipBlock">
                <p>
                  本页筛选<strong>截止日</strong>同时满足：<strong>MACD 绿转红</strong>（前一日柱≤0、当日柱&gt;0）
                  且当日为<strong>首板涨停（涨幅≥10%）</strong>；前一交易日非涨停。
                </p>
                <p><strong>不</strong>判断次日是否二板，续板请自行跟踪。区间二连板回测请用「历史回测」同名策略。</p>
                <p>不构成投资建议。</p>
              </div>
            </template>
        ...
        <div class="subtitle">
          与历史模拟交易「内置策略选股」中「连续打板」口径一致；可选手动执行或查询最新落库结果。
        </div>
      </div>
      <div class="actions">
    ...
    <el-card class="card" shadow="never">
      ...
      <div class="note">
        <p>
          <strong>入选</strong>：截止日 MACD 绿转红 + 首板（涨幅≥10%）。
          <strong>不入选</strong>：二板、涨幅仅 9.5%～10% 未封板、非绿转红、非主板或 ST。
          <strong>回测卖出</strong>（或关系）：最高收盘回落≥5%，或 MACD 红转绿。
        </p>
      </div>
    </el-card>
    ...
    <el-card class="card" shadow="never">
      <template #header>
        <div class="cardTitle">筛选结果（首板 + 绿转红）</div>
      </template>
      ...
      <el-table :data="items" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column label="信号" min-width="140">
          <template #default>
            <el-tag size="small" type="danger">绿转红+首板</el-tag>
          </template>
        </el-table-column>
      </el-table>

      <div v-if="!loading && items.length === 0" class="empty">
        截止日下暂无「MACD 绿转红且首板涨停」的股票，或数据尚未同步。
      </div>
    </el-card>
  </div>
</template>
...
const STRATEGY_ID = 'lian_xu_da_ban'
