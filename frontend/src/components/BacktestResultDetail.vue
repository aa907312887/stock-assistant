          ...
          <el-table :data="trades" stripe size="small" v-loading="tradesLoading">
            ...
            <el-table-column label="触发日" width="110">
              <template #header>
                <span>触发日</span>
                <el-tooltip
                  content="形态或信号触发日（如曙光初现的阳线日、早晨十字星第三根阳线日、红三兵三连阳最后一日 T）；红三兵与早晨十字星（简化版）买入多为 T+1 开盘价，可能与触发日相差一日"
                  placement="top"
                >
                  <el-icon class="hint-icon-sm"><QuestionFilled /></el-icon>
                </el-tooltip>
                ...
                <el-tooltip placement="top">
                  <template #content>
                    <div style="max-width: 280px">
                      已平仓：单笔收益率相对买入价。止损因策略而异（如曙光初现约−10%、早晨十字星/红三兵约−8%）；早晨十字星（简化版）为止盈/止损均在触发日按收盘价（阈值约+10%/−8%）。止盈在早晨十字星与红三兵多为「涨幅≥15%后从最高价回落约5%」按当日收盘价；曙光初现等可能为固定比例止盈；以离场原因为准。
                    </div>
                  </template>
                  <el-icon class="hint-icon-sm"><QuestionFilled /></el-icon>
                </el-tooltip>
