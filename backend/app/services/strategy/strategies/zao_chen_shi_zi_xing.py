...
def run_morning_star_backtest(
    ...
    *,
    ...
    """
    早晨十字星核心回测逻辑（可被外部策略复用）。

    参数：
    - extra_filter: 可选回调 (stock_code: str, bar_t: Any) -> bool
      在形态判断通过后调用，返回 False 则跳过该信号。
      默认 None 表示不过滤。
    """
    ...
    for code, bars_list in stock_bars.items():
        ...
        for i in range(min_i, len(bars_list)):
            ...
            # 额外过滤（如 PE 百分位、ROE 等）
            if extra_filter is not None and not extra_filter(code, bar_t):
                continue
...
class ZaoChenShiZiXingStrategy(StockStrategy):
    ...
    def execute(self, *, as_of_date: date | None = None) -> StrategyExecutionResult:
        ...
        try:
            ...
            result = run_morning_star_backtest(db, start_date=dd, end_date=dd, p=p)
    ...
    def backtest(self, *, start_date: date, end_date: date) -> BacktestResult:
        ...
        db = SessionLocal()
        try:
            return run_morning_star_backtest(db, start_date=start_date, end_date=end_date, p=p)
        finally:
            db.close()
